...
export default function Cover({ children }: { children: React.ReactNode }) {
  ...
  return (
    <motion.div
      ...
      className="relative w-full bg-[radial-gradient(300%_50%_at_top,_#98dbd5,_white)] rounded-xl shadow-xl z-10 transition-all duration-300 "
    ...
    >
      <div className="flex sticky top-0 py-2 px-4 items-center justify-between">
        ...
        <Button isIconOnly variant="light" onPress={toggleRight}>
          <Icon
            icon={
              rightOpen ? "solar:square-arrow-right-linear" : "solar:pen-bold"
            }
            fontSize={25}
            className="text-gray-700 hover:scale-110 transition-transform"
          />
